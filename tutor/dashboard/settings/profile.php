...
if ( ! empty( $user->first_name ) ) {
	...
	$public_display['display_lastfirst'] = $user->first_name;
}

if ( ! in_array( $user->display_name, $public_display ) ) { // Only add this if it isn't duplicated elsewhere
	$public_display = array( 'display_displayname' => $user->nickname ) + $public_display;
}
	...
	<form action="" method="post" enctype="multipart/form-data">
		...
		<div class="tutor-col-12 tutor-col-sm-6 tutor-col-md-12 tutor-col-lg-6 tutor-mb-32">
			...
			<div class="tutor-col-12 tutor-col-sm-6 tutor-col-md-12 tutor-col-lg-6 tutor-mb-32">
				...
				<input class="tutor-form-control" type="text" name="first_name" value="<?php esc_attr_e( $user->first_name ); ?>" placeholder="<?php esc_attr_e( 'Imię', 'tutor' ); ?>">
			</div>
		...
		<input class="tutor-form-control" type="hidden" name="last_name" value="<?php esc_attr_e( $user->last_name ); ?>" placeholder="<?php esc_attr_e( 'Last Name', 'tutor' ); ?>">
		<input class="tutor-form-control" type="hidden" pattern="[0-9]{3}-[0-9]{2}-[0-9]{3}" name="phone_number" value="<?php esc_html_e( filter_var( get_user_meta( $user->ID, 'phone_number', true ), FILTER_SANITIZE_NUMBER_INT ) ); ?>" placeholder="<?php esc_attr_e( 'Phone Number', 'tutor' ); ?>">
				<input class="tutor-form-control" type="hidden" name="tutor_profile_job_title" value="<?php esc_attr_e( get_user_meta( $user->ID, '_tutor_profile_job_title', true ) ); ?>" placeholder="<?php esc_attr_e( 'UX Designer', 'tutor' ); ?>">
		...
		<div class="tutor-row">
			<div class="tutor-col-12 tutor-col-sm-6 tutor-col-md-12 tutor-col-lg-6 tutor-mb-32">
				<label class="tutor-form-label tutor-color-secondary">
					<?php _e( 'Przedstawiaj mnie jako', 'tutor' ); ?>
				...
				<select class="tutor-form-select" name="display_name">
					...
					foreach ( $public_display as $id => $item ) {
						?>
								<option <?php selected( $user->nickname, $item ); ?>><?php esc_html_e( $item ); ?></option>
							<?php
					}
					?>
				</select>
